const STORAGE_KEY = 'activ-fitness-ui-state-v7';
  ...
  screens.home.innerHTML = `
    ...
    <section class="panel">
      ...
      <article class="hero workout-card" id="open-plan">
        ${cover ? `<img src="${imageFor(cover)}" alt="plan" onerror="this.src='${placeholderSvg}'"/>` : ''}
        <div class="hero-overlay">
          <small class="tag">${plan?.note ?? 'Von meinem Trainer'}</small>
          <h4>${plan?.title ?? 'Plan'}</h4>
          <span>${plan?.items.length ?? 0} Übungen · ${plan?.calories ?? 305} Kcal</span>
        </div>
      </article>
      ...
      ${historyCards || '<article class="card"><p class="muted">Noch kein Training abgeschlossen.</p></article>'}
    ...
    ${state.showQuickActions ? `
      ...
      <aside class="quick-sheet">
        ...
        <button data-action="record">🏃 Training erfassen <span>›</span></button>
        <button data-action="courses">📅 Kurse <span>›</span></button>
        <button data-action="history">⟲ Trainingsverlauf <span>›</span></button>
        <button data-action="locations">📍 Standorte <span>›</span></button>
      </aside>
    ` : ''}
  `;
...
function renderCourses() {
    ...
    screens.builder.innerHTML = `
      ...
      <section class="panel calendar-panel">
        <div class="chips-scroll">
          <button class="chip">Filter</button><button class="chip">Alle Kurse</button><button class="chip">Heimstudio</button><button class="chip">Alle Kategorien</button>
        </div>
        <div class="days-scroll">
          ${days.map((d) => `<button class="day-chip ${d === state.selectedCourseDay ? 'active' : ''}" data-day="${d}">${d.replace(' ', '<br/>')}</button>`).join('')}
        </div>
  ...
  screens.builder.innerHTML = `
    <header class="hero-header red">
      <div class="header-top"><div class="avatar">AS</div><button class="plus-btn" id="open-actions-courses">+</button></div>
    </header>
    <section class="panel">
